...
<html>
 ...
 <meta charset = "UTF-8">
    ...
    <style>
      ...
      label, input[type="text"], input[type="number"], input[type="date"], input[type="time"], input[type="submit"] {
         display: block;
         margin: 10px;
         padding: 10px;
         width: 80%;
...
<body>
   ...
   <h1>Como Doar</h1>
    ...
    <form action = "" method="post">
        ...
        <input type = "hidden" name = "data" value = "<?php date_default_timezone_set('America/Sao_Paulo'); $hoje = date('Y-m-d'); echo $hoje;?>">
        <input type = "hidden" name = "horas" value = "<?php date_default_timezone_set('America/Sao_Paulo'); $today = date("H:i:s"); echo $today;?>">
    ...
    <input type="text" name="nrcontaedigito" placeholder="Digite o número da conta e dígito"><br>
     ...
     include_once "conexao.php";
        ...
        $sql = $conn->prepare("INSERT INTO `publicação` (`id_causa`, `usuariopostagem_id`,`qt_meta`, `ds_ocorrido`, `dt_date`, `hr_hora`, `ds_cortesia`) 
                               VALUES (NULL, '$usuario', '$meta', '$ocorrido', '$data', '$tempo', '$cortesia')");
...
$queryListagem = "SELECT publi.id_causa, user.nm_usuario, publi.ds_ocorrido, publi.qt_meta, publi.dt_date, publi.hr_hora, publi.ds_cortesia, chave_pix.nr_chave_cpf, dados_bancarios.nm_banco, dados_bancarios.nr_agencia, dados_bancarios.nr_conta_e_digito
