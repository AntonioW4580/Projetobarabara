...
$buscarArquivos = $conn->prepare("SELECT ds_arquivo, causaarquivo_id FROM arq_midia WHERE cadastromidia_id = ?");
...
if ($buscarArquivos->rowCount() > 0) {
    while ($rowArquivo = $buscarArquivos->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<img src='uploads/$nome_arquivo' alt='Imagem Anexada'>";
